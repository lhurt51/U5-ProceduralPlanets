...
public class Erosion : MonoBehaviour
{
    ...
    // Used to prevent carry cap getting too close to zero on flatter terrain
    ...
    int currentSeed;
    int currentErrosionRadius;
    int currentMapSize;

    // Initialize creates a System.Random object and precomputes indices and weights of erosion brush
    void Initialize(int mapSize, bool resetSeed)
    {
        ...
        if (erosionBrushIndices == null || currentErrosionRadius != erosionRadius)
        {
            InitializeBrushIndices(mapSize, erosionRadius);
            currentErrosionRadius = erosionRadius;
    ...
    public void Erode(float[] map, int mapSize, int numIterations = 1, bool resetSeed = false)
    {
        ...
        for (int iteration = 0; iteration < numIterations; iteration++)
        {
            ...
            for (int lifetime = 0; lifetime < maxDropletLifetime; lifetime++)
            {
                ...
                // Calculate droplet's offset inside the cell (0, 0) = at NW node, (1, 1) = SE node
                float cellOffsetX = posX - nodeX;
                float cellOffsetY = posY - nodeY;

                // Calculate droplets height and direction of flow with bilinear interpolation of surrounding heights
                ...
                // Stop simulating droplet if its not moving or has flowed over edge of map
                if ((dirX == 0 && dirY == 0) || posX < 0 || posX >= mapSize - 1 || posY < 0 || posY >= mapSize - 1) break;

                // Find the droplet's new height and calculate the delta Height 
                ...
                {
                    // If moving uphill (delta > 0) try fill up to the current height, otherwise deposit a fraction of the excess sediment
                ...
                else
                {
                    ...
                    for (int brushPointIndex = 0; brushPointIndex < erosionBrushIndices[dropletIndex].Length; brushPointIndex++)
                    {
                        ...
                        float weighedErodeAmount = amountToErode * erosionBrushIndices[dropletIndex][brushPointIndex];
